...
func Create_files(problem_name string, input_data []string, output_data []string) {
	...
	if err != nil {
		fmt.Println("[Error] Could not get the required dir path")
		panic(err)
	}
	if len(input_data) != len(output_data) {
		panic("length of input_data and output_data are not same in the data given to create_file")
	}
	fmt.Println("Creating a new dir in ", dir, " with the name", problem_name, "with permission: rwxrwxr-x")
	...
	for i := range input_data {
		...
		if err != nil {
			fmt.Println("[Error]: Writing to input file failed")
			panic(err)
		}
		...
		if err != nil {
			fmt.Println("[Error]: Writing to output file failed")
			panic(err)
		}
	}
}
